"use client";
import { useCreateContainerType } from "@/app/[lang]/(protected)/freight-rate-library/services/freightRateService";
...
import { ActiveOverlay, ActiveOverlayType, HandsontableProps } from "@/shared/types/custom-handsontable";
import { loadGoogleMaps } from "@/shared/utils/loadGoogleMaps";
import { HotTable } from "@handsontable/react";
import Handsontable from "handsontable";
import { registerAllModules } from "handsontable/registry";
import React, { useCallback, useEffect, useMemo, useRef, useState, } from "react";
import { getColumnHeaders, getColumnHeadersRenderer, getDataAtRow, getUpdateOverlayPosition, highlightCell, onCancelAddRow, onConfirmAddRow, onConfirmComment, onConfirmEdit } from "../../helpers/handsontableHelpers";
...
const CustomHandsontable: React.FC<HandsontableProps> = ({
    ...
    const updateOverlayPosition = useCallback(
        ...
        [hotRef, wrapperRef, headers, newRows, failedRows, loadingRows, setOverlayPositions]
    );
    ...
    return (
        <div className="relative-wrapper" ref={wrapperRef}>
            ...
            <FilterDropdown
                ...
                columnValues={activeFilter !== null ? getColumnValues(activeFilter) : []}
                selectedValues={
                    activeFilter !== null
                        ? columnFilters[activeFilter] || new Set(getColumnValues(activeFilter))
                        : new Set()
                }
            ...
            {activeOverlay && (
                <RowOverlay
                    ...
                    onConfirm={() => {
                        ...
                        if (activeOverlay.type === ActiveOverlayType.EDIT) {
                            onConfirmEdit(activeOverlay, handleConfirmEdit, commentRef, hotRef, removeEditOverlay);
                        }
                        else if (activeOverlay.type === ActiveOverlayType.CELL_COMMENT || activeOverlay.type === ActiveOverlayType.ROW_COMMENT) {
                            onConfirmComment(activeOverlay, handleConfirmComment, commentRef, hotRef, removeEditOverlay)
                        }
                        setActiveOverlay(null);
                        commentRef.current = "";
                    }}
                    ...
                    isCommentRequired={
                        activeOverlay.type === ActiveOverlayType.CELL_COMMENT || activeOverlay.type === ActiveOverlayType.ROW_COMMENT
                            ? true
                            : isCommentMandatory
                    }
                />
            ...
            {Array.from(new Set([...newRows, ...failedRows, ...loadingRows])).map((rowIndex) =>
                overlayPositions[rowIndex] ? (
                    <RowOverlay
                        ...
                        onConfirm={(idx) =>
                            onConfirmAddRow(idx, setNewRows, commentRef, onConfirmRow, hotRef, setAddRowInprogress, removeEditOverlay)
                        }
                        onCancel={(idx) =>
                            onCancelAddRow(idx, setNewRows, onCancelRow, hotRef, setAddRowInprogress, removeEditOverlay)
                        }
