...
import { ActiveOverlayType, type ActiveOverlay } from "@/components/common/handsontable/types";
...
interface HandleAfterChangeParams {
    ...
    highlightCell: (hot: any, row: number, col: number, isInvalid?: boolean) => void;
...
export function getHandleAfterChange({
    ...
    return (changes: any, source: any) => {
        if (source === "edit" && changes) {
            ...
            hot.batchRender(() => {
                changes.forEach(([row, prop, oldValue, newValue]: [number, number, string, string]) => {
                    const colIndex = typeof prop === "number" ? prop : colNameToIndex[prop];
                    ...
                    if (oldValue == newValue || !activeOverlay || activeOverlay.type !== ActiveOverlayType.EDIT) return;
...
export function getHandleAfterBeginEditing({
    ...
    const beginEdit = (hot: any, row: number, col: number) => {
        ...
        if (relations && highlightCell && setHighlightedCells && colNameToIndex && headers) {
    ...
    const scheduleOverlayUpdate = (
        ...
        requestAnimationFrame(() => {
            ...
            setActiveOverlay({ type: ActiveOverlayType.EDIT, row, col, oldValue, top, left });
        });
    };
    ...
    return async (row: number, col: number) => {
        ...
        if (!hasEditPermission && (!activeOverlay || activeOverlay.type !== ActiveOverlayType.EDIT) && !confirmationModalVisible) {
...
export function getAfterGetRowHeader({
    ...
    return (row: number, th: HTMLElement) => {
        ...
        if (failedRows) {
            th.parentElement?.classList.toggle(
                "failed-row",
                failedRows?.has(row)
            );
        }
...
export function getHandleContextMenuCallback({
    ...
    return (key: string, selection: any[]) => {
        ...
        if (key === "addRowComment") {
            ...
            setActiveOverlay({ type: ActiveOverlayType.CELL_COMMENT, row, col, top, left });
        }
    };
}
