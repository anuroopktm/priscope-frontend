...
import { useGetModuleImportErrorFile, useListModuleImports, useListModuleImportSummaryCount, useListExport, useGetExportedFile } from "@/shared/services/commonService";
import { FileDetailsModalProps, SimplifiedExport } from "@/shared/types/file-modal";
...
const FileDetailsModal: React.FC<FileDetailsModalProps> = ({ open = true, onClose, showLoader, showSnackBar, module, filterOptions, defaultTab = "uploaded" }) => {
    const [alignment, setAlignment] = useState(module === "fx_rate" ? filterOptions[0]?.value : defaultTab || "");
                        ...
                        uploads = res.uploads
                            ...
                            .map((item: any) => {
                                ...
                                return {
                                    ...
                                    status: (item.status === FILE_UPLOAD_STATUS.SUCCESS || item.status === FILE_UPLOAD_STATUS.PROCESSED) ? "Success" : item.status,
                                    ...
                                    errorFile: (item.status === FILE_UPLOAD_STATUS.PROCESSED || item.status === FILE_UPLOAD_STATUS.FAILED),
                                    hasUploadData: (item.status === FILE_UPLOAD_STATUS.SUCCESS || item.status === FILE_UPLOAD_STATUS.PROCESSED || item.status === FILE_UPLOAD_STATUS.FAILED)
                                };
                            });
                    }
    ...
    return (
        ...
        >
            <DialogTitle>
                ...
                >
                    <Typography variant="h6" sx={{ fontWeight: 600, color: theme.custom.textColor }}>
                        Files
                    </Typography>
                            ...
                            >
                                ...
                                >
                                    <Chip
                                        ...
                                        size="small"
                                        sx={{
                                            backgroundColor: file.status === "Success" ? "#1FC16B1A" : "#FB37481A",
                                    ...
                                    <Box sx={{ display: "flex", alignItems: "center", gap: "4px" }}>
                                        <Image src={UserIcon} alt="User" width={20} height={20} style={{ padding: 2 }} />
                                        <Typography variant="body2" color="#1A2B44">
                                            {file.uploadedBy}
                                        </Typography>
                                    </Box>
                                    <Box sx={{ display: "flex", alignItems: "center", gap: "4px" }}>
                                        <Image src={ClockIcon} alt="Clock" width={20} height={20} style={{ padding: 2 }} />
                                        <Typography variant="body2" color="#1A2B44">
                                            {file.uploadDate} {file.uploadTime}
                                        </Typography>
                                    </Box>
                                    <Box sx={{ display: "flex", alignItems: "center", gap: "4px" }}>
                                        <Image src={CalendarIcon} alt="Calendar" width={20} height={20} style={{ padding: 2 }} />
                                        <Typography variant="body2" color="#1A2B44">
                                            {file.updatedDate}
                                        </Typography>
                                    </Box>
                                ...
                                {(file.hasUploadData) && <Box sx={{ mt: 2 }}>
                                ...
                                <React.Fragment key={file.id}>
                                    <ListItem
                                        ...
                                        sx={{
                                            ...
                                            gap: 2,
                                            borderRadius: '5px',
                                            backgroundColor: selectedDownloadIndex === index ? '#3B9EDC1A' : ''
                                        }}
                                    >
                                        <Box flex={1}>
                                            <Typography
                                            ...
                                            <Stack direction="row" spacing={1} alignItems="center">
                                                ...
                                                <Stack direction="row" spacing={1} alignItems="center">
                                                    ...
                                                    <Typography
                                                        variant="body2"
                                                        sx={{ fontSize: '14px', fontWeight: 400, color: theme.custom.textColor }}
                                                    >
                                                ...
                                                <Stack direction="row" spacing={1} alignItems="center">
                                                    ...
                                                    <Typography
                                                        variant="body2"
                                                        sx={{ fontSize: '14px', color: theme.custom.textColor }}
                                                    >
                                        ...
                                        {file.status === 'Success' && <Typography
                                            component="span"
                                    ...
                                    {index < exportedData.length - 1 &&
                                        !(selectedDownloadIndex === index || selectedDownloadIndex === index + 1) && (
...
const FileStatsLoader = ({ fileId, module }: { fileId: string, module: any }) => {
    const { data, isLoading, isError } = useListModuleImportSummaryCount(fileId, module);
