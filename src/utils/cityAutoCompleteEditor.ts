...
class CityAutocompleteEditor extends Handsontable.editors.TextEditor {
    ...
    prepare(row: number, column: number, prop: string | number, TD: HTMLTableCellElement, originalValue: any, cellProperties: any) {
        ...
        this.service?.getPlacePredictions(request, (predictions, status) => {
            if (status === google.maps.places.PlacesServiceStatus.OK && predictions && predictions.length) {
                if (this.options.mode === 'country') {
                    this.suggestions = predictions.filter(p => p.types.includes("country"));
                } else {
    ...
    handleKeyDown(e: KeyboardEvent) {
        ...
        if (e.key === "ArrowDown") {
            e.preventDefault();
            this.setActive(Math.min(this.activeIndex + 1, this.dropdown.childElementCount - 1));
    ...
    handleOutsideClick(evt: MouseEvent) {
        if (!this.dropdown) return;
        const path = (evt.composedPath?.() || (evt as any).path || []) as HTMLElement[];
