...
interface TariffRateInitialPageProps {
  onImportData: () => void;
  onAddManually: () => void;
  hasImportPermission: boolean;
}

// Card data configuration for tariff rates (when list API returns empty array)
const TARIFF_RATE_CARD_CONFIGS = (): readonly CardConfig[] =>
  [
    {
      id: "csv-upload",
      title: (
        <span
          dangerouslySetInnerHTML={{
            __html: "Hey, It seems like you're new here",
          }}
        />
      ),
...
const ActionCard = React.memo<ActionCardProps>(({ config, onAction }) => {
  const { id, title, buttonText, image, imageAlt, imageStyle } = config;
...
const TariffRateInitialPage: React.FC<TariffRateInitialPageProps> = ({
  onImportData,
  onAddManually,
...
export default TariffRateInitialPage;
